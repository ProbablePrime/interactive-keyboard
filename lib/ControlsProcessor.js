...
function ControlsProcessor(config) {
	this.joyStickHandler = require('./handlers/mouse/robotjs');
...
ControlsProcessor.prototype.processTactile = function (controlState, users, tactileState) {
	...
	// DEBUG
	if (control.label.toLowerCase() === 'applause' && decision.action) {
		// seen = true;
	}
	if (control.label.toLowerCase() === 'applause' && seen) {
		// console.log(tactileState);
	}
	// #############
...
ControlsProcessor.prototype.processJoyStick = function (state, users, joyStickState) {
	var result = this.joyStickconsensus(joyStickState, users, this.config, this.joyStickHandler);
	if (result) {
...
ControlsProcessor.prototype.constrainMouse = function(mouseBounds) {
	this.joyStickHandler.constrainMouse(mouseBounds);
};
