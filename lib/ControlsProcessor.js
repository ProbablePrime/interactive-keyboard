...
function ControlsProcessor(config) {
	...
	if (this.config.mouseEnabled) {
		...
		if (this.config.windowTarget) {
			this.windowHandler = require('./handlers/window/windoze.js');
...
ControlsProcessor.prototype.process = function (report, controlState) {
	const tactileResult = report.tactile.map(tactile => this.processTactile(controlState, report.users, tactile))
	.filter(value => value !== undefined);

	const joystickResult = report.joystick.map(joystick => this.processJoyStick(controlState, report.users, joystick))
	.filter(value => value !== undefined);
...
ControlsProcessor.prototype.handleClick = function (button) {
