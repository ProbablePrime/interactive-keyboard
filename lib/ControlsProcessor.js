...
function ControlsProcessor(config) {
	...
	if (this.config.custom) {
		this.customHandlers = Object.assign({}, this.config.custom);
	}
...
ControlsProcessor.prototype.handleCustom = function(type, controlState) {
	let custom = this.customHandlers[type][controlState.id];
	if (typeof custom === "string") {
		this.customHandlers[type][controlState.id] = require(`./handlers/custom/${custom}.js`);
		custom = this.customHandlers[type][controlState.id];
		custom.setup(this.config).then(res => {
			custom.apply(controlState);
		});
	} else {
		custom.apply(controlState);
	}
...
ControlsProcessor.prototype.processJoyStick = function (controlState, users, joyStickState) {
	...
	if (this.customHandlers && this.customHandlers.joystick && this.customHandlers.joystick[joyStickState.id]) {
		this.handleCustom('joystick', joyStickState);
		return;
	}
...
ControlsProcessor.prototype.handleClick = function (button, action) {
	if (button.search('left') !== -1 || button.search('draw') !== -1) {
