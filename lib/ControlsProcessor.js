...
function ControlsProcessor(config) {
	...
	this.keyboardHandler = require(`./handlers/${config.handler}`);
	this.windowHandler = null;
	this.mouseHandler = null;

	if (this.config.mouseEnabled) {
		// Mouse requires robot-js atm
...
ControlsProcessor.prototype.processTactile = function (controlState, users, tactileState) {
	...
	if (control.action !== decision.action) {
		changed = true;
		if (control.isMouseClick() && this.config.mouseEnabled) {
...
ControlsProcessor.prototype.setKey = function (keyName, status) {
	...
	if (status) {
		this.keyboardHandler.press(keyName.toUpperCase());
	} else {
		this.keyboardHandler.release(keyName.toUpperCase());
	}
};
